...
<cfquery datasource="#application.DataSource#"  		 
	...
	order by <!--- area --->region.region, orgname, rank
</cfquery><!--- 
	<cfoutput>select infra_monthly.userid,it_con,staff_time,staff_job_desc,staff_recruit,
	staff_interview,staff_hired,partner_recruit,
	partner_maint,fiscal_voucher,fiscal_track, barriers, progress, success, 
	tcp_con, tcp_am, tcp_rm, tcp_sccm, tcp_rscm, tcp_ann, tcp_msm, tcp_ybm,
	evalstatus, web_maint, web_hit, web_comment,
...
<!--- <cfquery datasource="#application.DataSource#"   
	...
	select topic, tfocus, tdate, trecip, ttrain, tlen, tattend1, tattend2, partnerSpons, seq
...
<cfloop query="traindet">
	...
	<cfquery datasource="#application.DataSource#"  
	 ...
	 username="#application.db_username#" name="trainingRecipDet">
		...
		where num in <cfif trecip EQ "">(0)<cfelse>(#trecip#)	</cfif>	
		and year2=#session.fy#
	</cfquery>
	<cfquery datasource="#application.DataSource#"  
	 password="#application.db_password#" 
	...
	<td colspan="2">Was your organization represented at any Tobacco Control Program meetings this month?
		...
		<input type="radio"  value="0"  <cfif not isDefined("QinfraDet.tcp_con") or (isDefined("QinfraDet.tcp_con") and QinfraDet.tcp_con EQ "0") or QinfraDet.tcp_con EQ ""> checked</cfif> disabled>No
			...
			<cfif isDefined("QinfraDet.tcp_con") and QinfraDet.tcp_con EQ "1">&nbsp;
				...
				<tr>
					<td><cfif session.fy LT 2007>TCP Annual Meeting<cfelse>TCP Statewide Meeting</cfif></td>
					<td>
				...
				<tr>
					<td><cfif session.fy LT 2007>Youth board meeting<Cfelse>Regional / Statewide Youth Meeting</cfif></td>
					<td>
...
<tr>
	...
	<cfif isdefined("qinfradet.evalstatus") and #qinfradet.evalstatus# is not ''>
	<cfquery datasource="#application.DataSource#"  
	 password="#application.db_password#" 
	 username="#application.db_username#" name="QEvalStatus">
	select descrip
	from  lu_eval_status
	where year2=#session.fy# and id = #qinfradet.evalstatus#
</cfquery>
</cfif>

<table border=".5" width="98.5%" cellspacing="0" title="local evaluation">
				<tr>
					<td colspan="2"><div align="center"><strong>Local Evaluation <cfoutput>(#thisRptMon#)</cfoutput></strong></div>
					</td>
				</tr>
				<tr>
					<td width="30%">What is the status of your local evaluation project?
					</td><td width="70%">
					<cfoutput>
					<cfif qinfradet.evalstatus is not ''>
		<select name="evalstatus" disabled="disabled">
		<option value="">#qevalstatus.descrip#</option>
		</select>
		<cfelse>
				<select name="evalstatus" disabled="disabled">
...
<!--- <cfselect name="evalStatus" query="QEvalStatus" display="descrip" value="id" selected="#QinfraDet.evalStatus#">
	</cfselect>  --->					</td>
				</tr>
				<tr>
					<td>Local level evaluation progress details:
					</td><td>#QinfraDet.llepd#
	</cfoutput>
					</td>
				</tr>			
			</table>
