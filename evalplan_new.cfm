...
<cfquery datasource="#application.DataSource#" 
	...
	<cfset form.date_complete = '#RecordDet.date_complete#'>	
...
<div align="center"><h2>Description of Evaluation Method</h2></div>
<br>
<script language="JavaScript">
...
Please provide a unique name for the evaluation method (e.g. POP ad observation, policy maker survey, faculty interviews, policy analysis/document review):
</td>
</tr>
<tr>
...
<tr>
	...
	<cfif isdefined("form.date_complete") and form.date_collecting NEQ "1/1/1900">
			<cfinput type="text" name="date_complete"  value="#form.date_complete#" >
		<cfelse>
			<cfinput type="text" name="date_complete" >
		</cfif>
	</td>
</tr>
</table>
...
<tr>
	<td colspan="3">
	There are several different approaches or designs that can be used to evaluate whether or not an intervention has produced an impact. Please select the option that most closely describes your evaluation approach. Please note that there is room to describe an alternative approach not listed.
	</td>
</tr>
<tr><td>&nbsp;</td></tr>
<tr><td>&nbsp;</td></tr>
...
<tr>
	...
	<textarea name="other_eval_txt" cols="140" rows="6"><cfif isDefined("form.other_eval_txt")><cfoutput>#form.other_eval_txt#</cfoutput></cfif></textarea>
	</td>
</tr>
...
<table class="boxN">
	...
	<tr>
		...
		<td colspan="4"><a name="showdmeth"></a>What is the data collection method that you will use?</td>
	</tr>
	<tr>
		<td colspan="2">&nbsp;</td>
		<td ><input type="radio" name="primary_type" onclick="showdmeth();" value="1"<cfif isDefined ("form.primary_type") and form.primary_type EQ "1"> checked</cfif>>Survey, face-to-face/hand-out</td>
		<td><input type="radio" name="primary_type" onclick="showdmeth();" value="2"<cfif isDefined ("form.primary_type") and form.primary_type EQ "2"> checked</cfif>>Observation</td>
	</tr>
	<tr>
		<td colspan="2">&nbsp;</td>
		<td ><input type="radio" name="primary_type" onclick="showdmeth();" value="3"<cfif isDefined ("form.primary_type") and form.primary_type EQ "3"> checked</cfif>>Survey, mail</td>
		<td><input type="radio" name="primary_type" onclick="showdmeth();" value="6"<cfif isDefined ("form.primary_type") and form.primary_type EQ "6"> checked</cfif>>Record/Document review</td>
	</tr>
	<tr>
		<td colspan="2">&nbsp;</td>
		<td><input type="radio" name="primary_type" onclick="showdmeth();" value="5"<cfif isDefined ("form.primary_type") and form.primary_type EQ "5"> checked</cfif>>Survey, telephone</td>
		<td><input type="radio" name="primary_type" onclick="showdmeth();" value="7"<cfif isDefined ("form.primary_type") and form.primary_type EQ "7"> checked</cfif>>Interview</td>
	</tr>
	<tr>
		<td colspan="2">&nbsp;</td>
		<td><input type="radio" name="primary_type"  onclick="showdmeth();" value="9"<cfif isDefined ("form.primary_type") and form.primary_type EQ "9"> checked</cfif>>Survey, e-mail/web based</td>
		<td><input type="radio" name="primary_type" onclick="showdmeth();" value="4"<cfif isDefined ("form.primary_type") and form.primary_type EQ "4"> checked</cfif>>Case study</td>
	</tr>
	<cfif isDefined("form.primary_type") and form.primary_type EQ 4>
	<tr><td>&nbsp;</td></tr>
	<tr>
		<td colspan="2">&nbsp;</td>
		<td align="right" valign="top">Which data collection method will you use for the case study?</td>
		<td>
		&nbsp;&nbsp;<input type="radio" name="casemeth" value="1"<cfif isDefined ("form.casemeth") and form.casemeth EQ 1> checked</cfif>>Observation<br>
		&nbsp;&nbsp;<input type="radio" name="casemeth" value="2"<cfif isDefined ("form.casemeth") and form.casemeth EQ 2> checked</cfif>>Record/Document Review<br>
		&nbsp;&nbsp;<input type="radio" name="casemeth" value="3"<cfif isDefined ("form.casemeth") and form.casemeth EQ 3> checked</cfif>>Interview<br>
		&nbsp;&nbsp;<input type="radio" name="casemeth" value="4"<cfif isDefined ("form.casemeth") and form.casemeth EQ 4> checked</cfif>>Survey<br>
		</td>
	</tr>
	...
	<tr>
		...
		<td><input type="radio" name="secondary_type" value="2"<cfif isDefined ("form.secondary_type") and form.secondary_type EQ "2"> checked</cfif>>Youth Tobacco Survey</td>
	</tr>
	<tr>
		<td colspan="2">&nbsp;</td>
		...
		<td><input type="radio" name="secondary_type" value="4"<cfif isDefined ("form.secondary_type") and form.secondary_type EQ "4"> checked</cfif>>X-Behavioral Risk Factor Surveillance</td>
	</tr>
	<tr>
		<td colspan="2">&nbsp;</td>
		<td valign="top"><input type="radio" name="secondary_type" value="5"<cfif isDefined ("form.secondary_type") and form.secondary_type EQ "5"> checked</cfif>>Adult Tobacco Survey</td>
	...
	<tr>
	<td colspan="4"><a name="numtimes"></a>
	...
	<input type="radio" name="multi_eval" onClick="numtimes();" value="1"<cfif isDefined ("form.multi_eval") and form.multi_eval EQ "1"> checked</cfif>> Yes &nbsp;&nbsp;&nbsp;&nbsp;
	<input type="radio" name="multi_eval" onClick="numtimes();" value="0"<cfif NOT isDefined("form.multi_eval") OR (isDefined ("form.multi_eval") and form.multi_eval NEQ "1")> checked</cfif>> No	
	...
	<tr>
	<td colspan="4">
	If yes, how many times will you be collecting the information using this method?
...
<table class="boxN">
	...
	<tr>
		<td>In this section, you will describe the source of the information that will be used for the evaluation:</td>
	</tr>
	
	<tr>
		<td>Sources of information can consist of people, groups, places, and/or events. 
		</td>
	</tr>
	...
	<tr>
		<td>Please describe your source (who or where you will get the information from).<br>
		Include important characteristics such as the significance of locations of people chosen, local political or historical context,
		past interventions, etc., to help us understand why you have chosen this source of information.
		
	</td>
	</tr>
	<tr>
		<td align="center">
		<textarea name="infosource" cols="140" rows="4"><cfif isDefined("form.infosource")><cfoutput>#form.infosource#</cfoutput></cfif></textarea>
		</td>
	</tr>
...
<tr>
	...
	include information about only one of them on this screen, and then click “Save” 
